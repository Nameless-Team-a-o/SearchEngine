...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(
    ...
    username = jwtService.extractUsernameFromAccess(jwt);  // Extract username instead of email

    if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
      UserDetails userDetails = this.userDetailsService.loadUserByUsername(username);  // Use username for authentication

      // for logout
      var user = userRepository.findByUsername(username).orElse(null);  // Fetch user by username
      if (user != null) {
